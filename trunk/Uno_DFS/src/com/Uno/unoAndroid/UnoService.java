...
public class UnoService extends Service {
	...
	private static String GOVERNOR_IP = "192.168.10.143";
	...
	private void NetworkMessageParser(Context c, String msg) {
		...
		if (argc == 1) {
			...
			intent.putExtra("GOVERNOR_MSG_STREAM_TO_UI", msg);
			UnoService.this.sendBroadcast(intent);
		}
		else if (argc == 2) {
			if (argv[0].equals("LOGIN")) {
				sendTcpPacket(getLocalIPAddr(), 5000, msg);				
			}
		}
		else if (argc == 3) {
    			...
    			if (DEVICE_LOCAL_LISTEN_IP != null) {
    				...
    				while (true) {
    					...
    					try {
    						...
    						NetworkMessageParser(mCtx, incomingMsg);
